...
$$
1 - \frac{\mathbf D_{11} + \mathbf D_{22}}{\sum_{i=1}^r \mathbf D_{ii}} = \frac{\sum_{i=1}^n \sum_{j=1}^p (\hat {\mathbf S}_{ij} - \mathbf S_{ij})^2}{\sum_{i=1}^n \sum_{j=1}^p \mathbf S_{ij}^2}
$$
...
sum(hec_svd$d[1:2]) / sum(hec_svd$d)
```
