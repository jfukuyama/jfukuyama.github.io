...
- If you want more detail on environments: _Advanced R_ by Hadley Wickham has a good [chapter](http://adv-r.had.co.nz/Environments.html) on them. That chapter makes a distinction between a function's _enclosing_ environment and its _binding_ environment, but for the purposes of this class we are going to assume those are always the same.
...
So for example, if we return to our steak-cooking example from a couple lectures ago, we might define the following function:
...
oddcount(x = c(0, 5))
oddcount_output <- oddcount(x = c(0, 5))
oddcount_output
```
...
The commands are executed in the function's _execution environment_.
...
When a function is called, its body is evaluated in an _execution environment_ whose parent is the function's _enclosing environment_.
...
f <- function(y) {
    ...
    cat("h's enclosing environment: ", "\n")
    print(environment(h))
    cat("parent of h's enclosing environment:", "\n")
