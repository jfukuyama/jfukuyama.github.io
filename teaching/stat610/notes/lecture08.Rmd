...
## Some extra things about pipes

- If you want to pipe the LHS to a position other than the first, you can use `.` in `magrittr` and `_` in base R.
- `.` can be used multiple times, `_` can only be used once, and it has to be a named argument, you can't use it positionally.
- `|>` requires parentheses on the RHS, but `%>%` doesn't if you don't have any extra arguments you need to pass in.

```{r}
# the magrittr pipe doesn't require you to have parentheses if you're
# just piping to the first position and no extra arguments
x %>% head
```
But `|>` does:
```{r}
...
Some examples of `.` and `_`:
```{r}
"[[:space:]]*and[[:space:]]*" %>% strsplit("cats and dogs", .)
"[[:space:]]*and[[:space:]]*" |> strsplit("cats and dogs", split = _)
```

Demonstratting that you need a argument name for `_`:
```{r}
"[[:space:]]*and[[:space:]]*" |> strsplit("cats and dogs", _)
```
