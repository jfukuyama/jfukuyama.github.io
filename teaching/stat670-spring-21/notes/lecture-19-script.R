...
ggbiplot(competitive_lda, groups = members$region) + xlim(c(-4, 3))
## predictive accuracy is not as good for this model
table(predict(competitive_lda)$class, members$region)
## look at one of the bills: who voted for it?
table(votes[,"s323.106.1999"], members$region)
subset(vote_descriptions, vote_id == "s323.106.1999")
