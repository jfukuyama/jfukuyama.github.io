...
- There are some likelihoods that would be easy to maximize if we just had a littel bit of extra data
...
In this model, the observed-data likelihood for one point is:
$$
...
Note: that this is not convex, so we can't apply the tools we used before
...
&= \sum_{i=1}^n (1 - E[z_i \mid Y = y, \theta= \theta^{(n)}]\log(\phi_{\theta^{(n)}_0}(y_i)) + E[z_i \mid  Y = y, \theta= \theta^{(n)}] \log(\phi_{\theta^{(n)}_1}(y_i)) +\\
