...
In addition, we have the problem of inverting the Hessian, this scales badly with $p$, the number of parameters. (Usually think of as $O(p^3)$, but [computer scientists](https://en.wikipedia.org/wiki/Strassen_algorithm) and [computers](https://www.deepmind.com/blog/discovering-novel-algorithms-with-alphatensor) have worked really hard and gotten $O(p^{2.4ish})$)
